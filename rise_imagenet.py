...
def main():
    ...
    image_path = Path('0_Edinburgh_Nat_Gallery.jpg')
    ...
    for class_idx in labels:
        ...
        base_name = f'{image_path.name}_{class_name(class_idx)}_n_masks_{n_masks}_pkeep_{p_keep}_res_{feature_res}'
        visualization.plot_image(relevance_map, utils.img_to_array(img)/255., heatmap_cmap='jet',
                                 output_filename=base_name + '.png', show_plot=False)
    np.savez_compressed(base_name+'.npz', relevances=relevances)
